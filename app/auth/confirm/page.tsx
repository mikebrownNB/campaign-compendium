 ...
 * /auth/confirm
 *
 * Handles email verification tokens for all auth flows:
 *   - signup confirmation
 *   - password recovery
 *   - invite acceptance
 *   - email address change
 *
 * Supabase sends token_hash + type via the "Send Email" hook.
 * We build the email link pointing here (app domain) rather than
 * directly to the Supabase project URL, then call verifyOtp() here
 * to exchange the token for a session before redirecting onward.
 *
 * Query params:
 *   token_hash  — the token_hash from the hook payload
 *   type        — signup | recovery | invite | email_change_new | email_change_current
 *   redirect_to — (optional) URL to send the user after verification
 */
...
export default function ConfirmPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.verifyOtp({ token_hash: tokenHash, type }).then(({ error }) => {
      ...
      // Otherwise honour redirect_to or fall back to home
